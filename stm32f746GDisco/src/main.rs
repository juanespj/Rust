...
fn main() -> ! {
    ...
    // let mut delaytouch = perif.TIM4.delay_us(&clocks);
    
    // let mut touch = ft5336::Ft5336::new(&i2c, 0x38, &mut delaytouch).unwrap();
    c1.draw(display).ok();
    c2.draw(display).ok();
    t.draw(display).ok();
    ...
    let mut figcolor = Rgb565::new(10, 80, 0);
    let mut delay = perif.TIM5.delay_us(&clocks);
let mut sign:bool=true;
    loop {
        ...
        //         Ok(n) => {
        //         let c3 = egcircle!(
        //             center = (n.x as i32, n.y  as i32),
        //             radius = 8,
        //             style = primitive_style!(fill_color = figcolor)
        //         );
        //         c3.draw(display).ok();
        //     }
        //         ,
