...
impl SteppedSimulation {
    ...
    pub fn from_files<P: AsRef<Path>>(path_raw: P, path_log: P) -> Self {
        ...
        let mut ppb = 1;
        if steps.len() > 0 {
            ppb = (simulation_points / steps.len() as i32) as usize;
        }
        SteppedSimulation {
            steps: steps,
            variables: variables,
    ...
    pub fn get_results(&self, id: usize) -> Vec<f64> {
        self.reals[id].clone()
    }
    /// Returns the VariableResult for one variable at a given step
    pub fn get_values_for_variable_at(
        &self,
        step: &Step,
        var: &SimulationVariable,
    ) -> Option<VariableResult> {
        match self.steps.iter().position(|r| r.eq(step)) {
            Some(position) => match self.variables.iter().position(|r| r.eq(var)) {
                Some(var_pos) => {
    ...
    fn get_value_block_at(&self, pos: usize) -> Vec<VariableResult> {
        ...
        for v in &self.variables {
            ...
            result.push(VariableResult::new(
                v,
                local_reals[start..(start + self.points_per_block)].to_vec(),
                local_imags[start..(start + self.points_per_block)].to_vec(),
            ));
    ...
    pub fn calculate_fitnesses(&self, var: &SimulationVariable) -> Vec<f64> {
        ...
        for step in &self.steps {
            let freq = self
                .get_values_for_variable_at(&step, &self.variables[0])
                .unwrap();
    ...
    pub fn find_averages_for_fitness(&self, var: &SimulationVariable) -> ([f64; 5], [f64; 5]) {
        ...
        let freq = self
            .get_values_for_variable_at(&self.steps[0], &self.variables[0])
            .unwrap();
        let (index_one_k, _) = self
            .get_values_for_variable_at(&self.steps[0], &var)
            .unwrap()
            .find_value_near_freq(&DataType::AbsoluteDecibel, &freq, 1000.0);
        let (index_one_h, _) = self
            .get_values_for_variable_at(&self.steps[0], &var)
            .unwrap()
            .find_value_near_freq(&DataType::AbsoluteDecibel, &freq, 100.0);
        for step in &self.steps {
            let values = self.get_values_for_variable_at(&step, &var);
            match values {
                Some(values) => {
                    ...
                    min_maxs.push(
                        1.0 / (values.max(&DataType::AbsoluteDecibel).1
                            - values.min(&DataType::AbsoluteDecibel).1),
                    );
                    most_lefts.push(
                        values
                            .get_data_point(&DataType::AbsoluteDecibel, 0)
                            .unwrap(),
                    );
                    most_rights.push(
                        values
                            .get_data_point(&DataType::AbsoluteDecibel, index_one_k)
                            .unwrap(),
                    );
                    at_100_hz.push(
                        values
                            .get_data_point(&DataType::AbsoluteDecibel, index_one_h)
                            .unwrap(),
                    );
                }
    ...
    pub fn find_with_resonance_at(
        ...
        let freq = self
            ...
            .unwrap();
        let freq_index = freq
            .get_data(&DataType::Real)
            .iter()
            .position(|b| b > &res_freq)
            .unwrap();
...
fn read_raw_file<P: AsRef<Path>>(
    path: P,
    variables: &mut Vec<SimulationVariable>,
    reals: &mut Vec<Vec<f64>>,
    imags: &mut Vec<Vec<f64>>,
) -> i32 {
...
fn read_description<P: AsRef<Path>>(path: P, variables: &mut Vec<SimulationVariable>) -> i32 {
    ...
    while state != DescriptionState::Binary {
        ...
        file.read_until(b'\n', &mut line).unwrap();
        let line = String::from_utf8(line).unwrap().replace("\0", "");
...
fn get_values<P: AsRef<Path>>(path: P, reals: &mut Vec<Vec<f64>>, imags: &mut Vec<Vec<f64>>) {
    ...
    while ct < (file_buf.len() - 15) {
        if !in_binary {
            //Wait for binary
            let k = &[file_buf[ct].clone()];

            buffer.push(k[0]);
            let tmpbuff = String::from_utf8(buffer.clone()).unwrap().replace("\0", "");
            //println!("tmpbuf{:?}", tmpbuff);
            if tmpbuff.contains("Binary:\n") {
                in_binary = true;
                println!("--- FOUND ---");
            }
