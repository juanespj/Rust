...
pub mod data;
pub mod objects;
use crate::datalogger::{LoggerCtrl, LoggerState, log_plot};
...
use crate::sys_tools::serial_port_ui;
...
use crate::services::{ServiceCtrl, servic_gui};
use crate::sys_tools::app_init;
use std::sync::mpsc;
use std::sync::{
    Arc, RwLock,
...
pub struct RenderApp {
    ...
    portlist: Vec<String>,
    #[serde(skip)]
    pub port_sel: (String, Vec<String>),
    #[serde(skip)]
    pub srvcrequest: (
        mpsc::Sender<ServicesRequest>,
        mpsc::Receiver<ServicesRequest>,
    ),
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub services: Arc<RwLock<ServiceCtrl>>,
    #[serde(skip)]
    pub services_state: HashMap<String, String>,
    #[serde(skip)]
    pub cwd: String,
}
...
impl Default for RenderApp {
    fn default() -> Self {
        Self {
            state: AppState::Launch,

            // Example stuff:
            toasts: Toasts::default()
                .with_anchor(egui_notify::Anchor::BottomRight)
                .with_margin(Vec2 { x: 10.0, y: 10.0 })
                .with_shadow(Shadow {
            ...
            msgchan: mpsc::channel::<(String, String, String)>(),
            structs: Config::default(),
            ...
            timedelta: None,
            srvcrequest: mpsc::channel::<ServicesRequest>(),
            services_state: HashMap::new(),
            services: Arc::new(RwLock::new(ServiceCtrl::default())),
            timer: Duration::new(0, 0),
            data_ready: 0,
            device_state: DeviceState::new(),
            sersys: SerSys::new(),
            port_sel: ("-".to_string(), vec![]),
            portlist: vec![],
            dataset: data::RawData {
                diag: (HashMap::new()),
                dataf: (HashMap::new()),
...
impl RenderApp {
    ...
    pub fn new(cc: &eframe::CreationContext<'_>, sys: Config) -> Self {
        // This is also where you can customized the look at feel of egui using
        // `cc.egui_ctx.set_visuals` and `cc.egui_ctx.set_fonts`.
        let mut app_struct = RenderApp::default();
...
fn post_rendering(app: &mut RenderApp, _window_size_px: [u32; 2], _frame: &eframe::Frame) {
    let keys: Vec<Keycode> = app.device_state.get_keys();
    for key in keys.iter() {
        match key {
            Keycode::A => {
                if app.objstate.contains_key("rbb") {
                    let mut rbb = app.objstate.get("rbb").unwrap().clone();
                    rbb.entry("a".to_string()).and_modify(|k| *k += 0.01);
                    objects::draw_rbb(&mut rbb, &mut app.objectlist);
                    app.objstate
                        .entry("rbb".to_string())
                        .and_modify(|k| *k = rbb);
                }
                app.cmd = CMDapp::UpdPrev;
            }
            Keycode::D => {
                if app.objstate.contains_key("rbb") {
                    let mut rbb = app.objstate.get("rbb").unwrap().clone();
            ...
            Keycode::S => {
                // println!("{:?}", self.sersys);
                if app.sersys.state == SerState::IDLE {
                    ...
                    if app.data_ready < 6 {
                        ...
                        log_plot(&mut app.logger);
                        println!("data{:?}", app.logger.data);
                        app.data_ready += 1;
                    }
                    app.logger.status.remove("read");
                }
    ...
    match app.msgs.ble_ch.1.try_recv() {
        Ok(data) => {
            println!("fromBLE: {:?}", data.status);
            app.blectrl = data.clone();
            if app.blectrl.status.contains_key("periph") {
                app.blectrl.blelist = app.blectrl.status.get("periph").unwrap().to_vec()
            }
        }
    ...
    match app.cmd {
        CMDapp::BLEmsg => {
            if let Err(_) = app.msgs.ble_ch.0.send(app.blectrl.clone()) {
